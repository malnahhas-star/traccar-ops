...
public class DistanceHandler extends BasePositionHandler {
    ...
    public void onPosition(Position position, Callback callback) {
        ...
        // --- MIRNINT: GPS-derived ground speed (km/h) as a switchable 'sensor' ---
        // Computed from displacement over time between consecutive fixes. Stored
        // alongside the device-reported speed so FleetBack can switch per vehicle.
        double groundSpeed = 0.0;
        if (last != null && position.getFixTime() != null && last.getFixTime() != null) {
            long dtMillis = position.getFixTime().getTime() - last.getFixTime().getTime();
            if (dtMillis > 0) {
                groundSpeed = distance * 3600.0 / dtMillis; // (m / ms) -> km/h
            }
        }
        position.set("groundSpeed", groundSpeed);
        // --- end MIRNINT ---
