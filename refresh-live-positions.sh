...
SELECT v.id, v.company_id, ds.lat, ds.lng,
       CASE WHEN ds.last_update < now()::timestamp - interval '24 hours' THEN 'inactive'
            ...
            -- battery-voltage fallback fabricated 'idle' for parked cars (a charged
            -- battery reads ~14V with the engine OFF) — dropped.
            WHEN ds.ignition = true AND ds.last_update >= now()::timestamp - interval '6 minutes' THEN 'idle' ELSE 'stop' END,
